// Enhanced Content Script v1.3.3 with dynamic send button detection
// Fixed for ChatGPT arrow button and Gemini mat-icon send button
...
// Wait for dynamic button to appear
    ...
    for (const selector of buttonSelectors) {
      try {
        ...
        if (trimmedSelector.includes('path[d*=')) {
          // Find buttons containing the path
          const buttons = document.querySelectorAll('button');
          for (const btn of buttons) {
            const path = btn.querySelector('path[d*="M8.99992"]');
            if (path && !btn.disabled) {
              console.log(`[${platform}] Found button with arrow path`);
              return btn;
            }
          }
          ...
          if (icon) {
            ...
            if (btn && !btn.disabled) {
              console.log(`[${platform}] Found button with mat-icon`);
              return btn;
            }
            // If mat-icon itself is clickable
            if (icon.onclick || icon.style.cursor === 'pointer') {
              console.log(`[${platform}] Found clickable mat-icon`);
              return icon;
            }
          }
        } else {
...
console.log(`[${platform}] Enhanced content script v1.3.3 loaded`);
