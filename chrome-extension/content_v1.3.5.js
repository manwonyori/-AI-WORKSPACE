// Enhanced Content Script v1.3.5 with Quill API and improved event handling
// Advanced handling for ChatGPT arrow button and Gemini mat-icon send button
...
const SELECTORS = {
  ...
  gemini: {
    ...
    // Added mat-icon selector for Gemini
    button: "mat-icon[fonticon=\"send\"], mat-icon.send-button-icon, button:has(mat-icon[fonticon=\"send\"]), button[aria-label*=\"Send\"], button[aria-label*=\"send\"], button[aria-label*=\"Run\"], button[title*=\"Send\"], button[title*=\"Run\"], button.send-button, mat-icon-button[aria-label*=\"Send\"]",
    container: "main, c-wiz, mat-sidenav-content, .main-content",
    useCtrlEnter: false,  // Changed to false since button appears after input
    waitForButton: true   // Wait for mat-icon button to appear
  },
  perplexity: {
...
async function inputForGemini(inputEl, text) {
  ...
  if (inputEl.classList && inputEl.classList.contains("ql-editor")) {
    ...
    try {
      // Try to access Quill API directly
      const quill = inputEl.__quill || window.quill || null;
      ...
      if (quill) {
        ...
        // Inform Angular of value change
        quill.root.dispatchEvent(new InputEvent('input', { bubbles: true, cancelable: true }));
        quill.root.dispatchEvent(new Event('change', { bubbles: true }));
        quill.root.dispatchEvent(new Event('blur', { bubbles: true }));
        
        console.log("[Gemini] Quill API text set successfully");
        return true;
      } else {
        // Fallback to DOM manipulation
        console.log("[Gemini] No Quill instance found, using DOM fallback");
        inputEl.focus();
        
        // Clear and set content with proper escaping
        const escapedText = text.replace(/</g, '&lt;').replace(/>/g, '&gt;');
        inputEl.innerHTML = `<p>${escapedText}</p>`;
        ...
        console.log("[Gemini] Quill DOM text set successfully");
        return true;
      }
    } catch (e) {
...
async function waitForButton(maxWait = 3000) {
  const sel = SELECTORS[platform];
  ...
  return new Promise((resolve) => {
    ...
    const checkForButton = () => {
      ...
      for (const selector of buttonSelectors) {
        try {
          ...
          // Special handling for Gemini mat-icon
          if (trimmedSelector.includes('mat-icon') || platform === 'gemini') {
            // The interactive target is usually the parent <button> of the mat-icon
            let sendBtn = document.querySelector('button mat-icon[fonticon="send"]')?.closest('button');
            
            // Try alternative selectors if theme differs
            if (!sendBtn) {
              sendBtn = document.querySelector('button[aria-label*="Send" i]') ||
                        document.querySelector('button[mattooltip*="Send" i]') ||
                        document.querySelector('mat-icon-button[aria-label*="Send" i]');
            }
            ...
            if (sendBtn) {
              ...
              if (isVisible) {
                // Additional check for Gemini - make sure it's not disabled
                const isDisabled = sendBtn.hasAttribute('disabled') || 
                                  sendBtn.classList.contains('disabled') ||
                                  sendBtn.getAttribute('aria-disabled') === 'true';
                
                if (!isDisabled) {
                  console.log(`[${platform}] Found Gemini send button`);
                  resolved = true;
                  return sendBtn;
                }
              }
    ...
    // Setup MutationObserver to watch for button appearance
...
async function sendMessage() {
  ...
  for (const selector of buttonSelectors) {
    try {
      ...
      } else if (trimmedSelector.includes('mat-icon')) {
        // Special handling for mat-icon - prefer parent button
        let sendBtn = document.querySelector('button mat-icon[fonticon="send"]')?.closest('button');
    ...
    try {
      switch(req.action) {
        ...
        case "inputAndSend":
          ...
          if (ok1) {
            // For platforms that need dynamic buttons, wait a bit for button to become active
            if (SELECTORS[platform]?.waitForButton) {
              console.log(`[${platform}] Waiting for button to become active after input...`);
              await new Promise(resolve => setTimeout(resolve, 300));
            }
            const ok2 = await sendMessage();
...
console.log(`[${platform}] Enhanced content script v1.3.5 loaded with Quill API support`);
