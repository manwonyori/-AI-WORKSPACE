// Enhanced Content Script v1.3.6 with reliable Gemini handling
// Advanced handling for ChatGPT arrow button and Gemini Angular/Quill integration
  ...
  if (inputEl.classList && inputEl.classList.contains("ql-editor")) {
    ...
    try {
      ...
      if (quill) {
        ...
        // Inform Angular of value change with proper sequence
        quill.root.dispatchEvent(new Event('input', { bubbles: true }));
        quill.root.dispatchEvent(new Event('change', { bubbles: true })); // Angular listens to 'change'
        quill.root.dispatchEvent(new Event('blur', { bubbles: true }));   // Trigger updates
        
        console.log("[Gemini] Quill API text set successfully");
        return true;
      } else {
        // Fallback to DOM manipulation for Quill
        console.log("[Gemini] No Quill instance found, using DOM fallback");
        inputEl.focus();
        
        // Set content directly (escaping not needed for Quill's innerHTML)
        inputEl.innerHTML = `<p>${text}</p>`;
        
        // Proper event sequence for Angular change detection
        inputEl.dispatchEvent(new Event('input', { bubbles: true }));
        inputEl.dispatchEvent(new Event('change', { bubbles: true })); // Important for Angular
        inputEl.dispatchEvent(new Event('blur', { bubbles: true }));   // Simulate focus loss
        
        console.log("[Gemini] Quill DOM text set successfully with Angular events");
        return true;
      }
    } catch (e) {
          ...
          if (trimmedSelector.includes('mat-icon') || trimmedSelector.includes('mat-icon-button') || platform === 'gemini') {
            ...
            if (sendBtn) {
              ...
              if (isVisible) {
                ...
                if (!isDisabled) {
                  console.log(`[${platform}] Found Gemini send button (aria-label priority)`);
                  resolved = true;
                  return sendBtn;
                }
              }
    ...
    try {
      switch(req.action) {
        ...
        case "inputAndSend":
          ...
          if (ok1) {
            // For Gemini, wait longer for Angular change detection
            if (platform === 'gemini') {
              console.log(`[${platform}] Waiting for Angular change detection...`);
...
console.log(`[${platform}] Enhanced content script v1.3.6 loaded with reliable Gemini handling`);
