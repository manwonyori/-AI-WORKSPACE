// Enhanced Content Script v1.3.7 with 3-step Gemini process
// Optimized for ChatGPT arrow button and Gemini Quill.js API integration
...
async function inputForGemini(inputEl, text) {
  ...
  // Handle Quill Editor with 3-step process
  if (inputEl.classList && inputEl.classList.contains("ql-editor")) {
    console.log("[Gemini] Using Quill editor 3-step process");
    
    try {
      // Step 1: Input text using Quill.js API (most reliable method)
      const quill = inputEl.__quill;
      
      if (quill) {
        console.log("[Gemini] ✅ Found Quill instance, using API (most stable method)");
        quill.setText(text);
        
        // Optional: Set cursor position to end
        const textLength = quill.getText().length;
        quill.setSelection(textLength, 0);
        
        console.log("[Gemini] Step 1 complete: Text input via Quill API");
        return true;
      } else {
        // Fallback if no Quill instance (less reliable)
        console.warn("[Gemini] ⚠️ Quill instance not found, using fallback method (less stable)");
        inputEl.focus();
        ...
        console.log("[Gemini] Step 1 complete: Text input via DOM (fallback)");
        return true;
      }
    } catch (e) {
      console.error("[Gemini] Step 1 failed: Quill editor error:", e);
      return false;
    }
  }
        ...
        try {
          ...
          if (trimmedSelector.includes('mat-icon') || trimmedSelector.includes('mat-icon-button') || platform === 'gemini') {
            ...
            let sendBtn = document.querySelector('button[aria-label="Send message"]') ||
                         document.querySelector('button.mat-icon-button:has(mat-icon[fonticon="send"])');
  ...
  (async () => {
    try {
      switch(req.action) {
        ...
        case "inputAndSend":
          if (platform === 'gemini') {
            console.log(`[Gemini] Starting 3-step message sending process...`);
          }
          
          const ok1 = await autoInput(req.text);
          if (ok1) {
            // For Gemini, wait for button to become active after text input
            if (platform === 'gemini') {
              console.log(`[Gemini] Text input complete, waiting for button activation...`);
              await new Promise(resolve => setTimeout(resolve, 200)); // Small delay for button activation
            } else if (SELECTORS[platform]?.waitForButton) {
              console.log(`[${platform}] Waiting for button to become active...`);
              await new Promise(resolve => setTimeout(resolve, 300));
            }
            
            const ok2 = await sendMessage();
            
            if (platform === 'gemini' && ok1 && ok2) {
              console.log(`[Gemini] ✅ All 3 steps completed successfully!`);
            }
            
            sendResponse({ success: ok1 && ok2, platform });
          } else {
            if (platform === 'gemini') {
              console.error(`[Gemini] ❌ Step 1 failed: Could not input text`);
            }
...
console.log(`[${platform}] Enhanced content script v1.3.7 loaded with 3-step Gemini process`);
