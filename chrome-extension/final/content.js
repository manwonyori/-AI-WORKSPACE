...
function watchForResponse(callback) {
    ...
    const interval = setInterval(() => {
        ...
        // Platform-specific completion detection
        let isComplete = false;
        
        if (STATE.platform === 'claude') {
            // Claude specific: Check for typing indicator
            const typingIndicator = document.querySelector('[data-is-streaming="true"]');
            if (!typingIndicator && currentLength > 50) {
                isComplete = true;
            }
        } else if (STATE.platform === 'chatgpt') {
            // ChatGPT specific: Check for regenerate button
            const regenerateBtn = document.querySelector('button:has(svg path[d*="M3.478"])');
            if (regenerateBtn && currentLength > 50) {
                isComplete = true;
            }
        }
        ...
        if (currentLength > 50 && currentLength === lastLength) {
            ...
            emptyCount++;
            if (emptyCount > 5) {
                console.log('⚠️ No response detected, retrying...');
            }
        } else {
            stableCount = 0;
            emptyCount = 0;
        ...
        // Timeout after 45 checks (90 seconds)
        if (checkCount >= 45) {
            clearInterval(interval);
            STATE.isProcessing = false;
            console.log('⏱️ Response watch timeout');
            if (callback && currentLength > 50) {
                console.log(`Sending partial response: ${currentLength} chars`);
                callback(response);
            } else {
                console.error('❌ No valid response detected');
            }
        }
    }, 2000);
}
    ...
    (async () => {
        ...
        try {
            switch (request.action) {
                ...
                case 'AUTO_RELAY':
                    console.log('🔄 Auto relay message received');
                    console.log(`Platform: ${STATE.platform}`);
                    console.log(`Message length: ${request.message?.length}`);
                    
                    try {
                        // Stop any current processing
                        const stopped = await clickStopButton();
                        if (stopped) {
                            console.log('⏹️ Stopped previous processing');
                            await wait(1500);
                        }
                        
                        // Clear input first
                        const config = PLATFORMS[STATE.platform];
                        const inputEl = await findElement(config.selectors.input);
                        if (inputEl) {
                            if (config.inputMethod === 'contenteditable') {
                                inputEl.innerHTML = '';
                            } else {
                                inputEl.value = '';
                            }
                        ...
                        if (!relayInput) {
                            console.error('❌ Failed to input message');
                            result.error = 'Failed to input message';
                            break;
                        }
                        
                        console.log('✅ Message input successful');
                        await wait(2000); // Give more time for input to register
                        ...
                        // Watch for response with enhanced callback
                        watchForResponse((response) => {
                            console.log(`📤 Sending response to background (${response.length} chars)`);
                            chrome.runtime.sendMessage({
                                ...
                                console.log('✅ Response sent to background:', ack);
                            }).catch(err => {
                                console.error('❌ Failed to send response to background:', err);
                            });
                        });
                        ...
                        console.error('❌ Auto relay error:', error);
